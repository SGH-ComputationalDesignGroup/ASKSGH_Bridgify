...
{
    public class sBeam : sElementBase
    {
        ...
        public List<sBeamResult> results { get; set; }

        public sBeam DuplicatesBeam()
        {
            sBeam newb = new sBeam();

            newb.objectGUID = this.objectGUID;

            newb.beamID = this.beamID;
            newb.parentGuid = this.parentGuid;
            newb.beamName = this.beamName;
            ...
            {
                newb.results = new List<sBeamResult>();
                foreach(sBeamResult re in this.results)
                {
                    newb.results.Add(re.DuplicatesBeamResult());
                }
            }
            return newb;
        }
        ...
        public sBeam(sLine axln, sXYZ upVec)
        {
        ...
        public sBeam(sNode n0, sNode n1)
        {
            this.node0 = n0;
            this.node1 = n1;
        ...
        public sBeam(sNode n0, sNode n1, sXYZ upVec)
        {
            this.node0 = n0;
            this.node1 = n1;
        ...
        public void UpdateLineLoadByPatternFactor_LinearAdditive(string pattern, double factor, ref sLineLoad comboLoad)
      {
        ...
        public sMesh ConstructBeamColorMesh(sRange dataRange, eColorMode colorMode, sRange threshold, double du)
        {
            ...
            {
                sBeamResult br_this = this.results[i];
                sBeamResult br_next = this.results[i + 1];
        ...
        public sRange GetBeamResultRange(eColorMode colorMode)
        {
            ...
            foreach (sBeamResult br in this.results)
            {
                ...
                {
                    foreach (sBeamVertexResult secRe in br.sectionResults)
                    {
